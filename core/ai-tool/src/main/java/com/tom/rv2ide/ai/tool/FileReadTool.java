...
public final class FileReadTool extends BaseTool {
    ...
    public ToolResult execute(JSONObject input, ToolContext context) {
        try {
            ...
            if (!file.exists()) {
                return error(ToolMessages.format(ToolMessages.FILE_READ_NOT_FOUND, FileToolPathPolicy.displayPath(context.getHomePath(), file)));
            }
            if (file.isDirectory()) {
            ...
            if (!hasKbRange) {
                if (fileLen > LARGE_FILE_THRESHOLD_BYTES) {
                    return error(ToolMessages.format(ToolMessages.FILE_READ_EXCEED_50KB, FileToolPathPolicy.displayPath(context.getHomePath(), file), fileLen / 1024, input.optString("file_path")));
                }
            ...
            if (startByte >= fileLen) {
                return error(ToolMessages.format(ToolMessages.FILE_READ_START_OUT_OF_RANGE, startKb, fileLen / 1024));
            }
            ...
            result.append(ToolMessages.format(ToolMessages.FILE_READ_RANGE_INFO, totalLines, startKb, endKb, fileLen / 1024));
    ...
    private void appendDirectory(StringBuilder builder, File dir, String parentPath, int[] count, ToolContext context) {
        ...
        for (File item : items) {
            if (count[0] >= MAX_DIRECTORY_ITEMS) {
                builder.append(ToolMessages.FILE_READ_DIR_TRUNCATED);
                return;
            }
