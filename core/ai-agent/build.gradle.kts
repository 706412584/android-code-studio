...
dependencies {
    api(projects.core.aiProtocol)
    api(projects.core.aiTool)
...
tasks.withType<Test>().configureEach {
    ...
    // 真实网关端到端测试（AgentLiveGatewayTest）默认跳过，靠这些系统属性开启。
    // Gradle 的 -D 只作用于 daemon 本身，必须显式转发到测试 JVM，否则属性读不到。
    listOf("ai.live.baseUrl", "ai.live.apiKey", "ai.live.model").forEach { key ->
