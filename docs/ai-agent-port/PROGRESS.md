...
- **授权粒度按工具分类决定**，因为「参数里什么才是危险的部分」因工具而异：
  shell 取**归一化后的整条命令**，文件写取**路径**，其余取**参数摘要**。
  （初版按命令「首词」授权是错的，见 §3.17——首词相同不代表破坏力相同。）
- **`file_delete` 的 scope 必须与「实际删除集合」逐字相同**——它接受 `paths` 数组
  **以及** `file_path` / `path` 单值，只按其中一种提取会让授权范围窄于执行范围，
  即一次点击放行了用户没看到的删除。因此 `ToolPermissionRule` 直接调用
  `FileDeleteTool.collectPaths`，两者共用同一个取路径方法，从结构上排除漂移。
- **绝不产生「空 scope」规则**——空 scope 会拼出 `<tool>\0`，它对**任意参数**都匹配。
  没有可判别字段时改取参数摘要（`args:<sha256 前 8 字节>`），语义是「只有参数完全相同的
  那一次调用才复用」。宁可多确认一次，也不做静默的无限放行。
- **规则键用 NUL 作分隔符**：工具名与 scope 都是受限字符集，NUL 不可能出现其中，
...
**测试**：`ToolPermissionRuleTest`（粒度边界、别名归一、NUL 无歧义、多路径覆盖、
非法 JSON 降级）。四个 AI 模块共 217 条全绿。

> ⚠️ 本节初版的「首词粒度」与「空 scope 退化」两处设计**是错的**，已被 §3.17 修正。
> 保留此处记录是因为教训本身有价值：两条错误都配有当时**全绿**的测试，
> 而测试之所以没发现，是因为它们把实现行为当成了期望行为——
> `assertEquals(gitStatus, gitPush)` 把「一次放行等于放行整类」这个提权路径钉成了「正确」。
> 写安全相关的测试时，期望值必须来自威胁模型，不能来自当前实现。
...
### 3.13 P1-10 消息操作与导出（#13）

**新文件**：`core/ai-agent/.../agent/export/ConversationExporter.java`。
**改动**：助手面板标题栏新增「复制」按钮（导出当前列表为 Markdown 到剪贴板）。

**关键设计决策**：

- **导出 Markdown 而非 PDF/PNG**。Markdown 是纯文本，任何地方都能打开与编辑，体积小。
  PDF/PNG 需要渲染管线与字体嵌入，收益（外观固定）远小于复杂度；需要 PDF 时用户可用
  任何 Markdown 工具转换。
- **过程信息默认折叠为细节块**。一次运行可能产生几十次工具调用，全部展开会把真正的
  结论淹掉。默认只输出工具名与状态标记，展开由阅读者决定。
- **截断必须注明**。工具输出可能极大（读文件、构建日志），静默截断会让读者以为内容
  就这么多——这比不截断更糟。
- **导出器只依赖条目、不依赖 `EntryLocation`**。导出只需要内容，不需要字节偏移与序号。
  依赖更窄的类型让导出器与日志的存储表示解耦，也让它可测（`EntryLocation` 的构造器是
  包级私有）。持有位置列表的调用方用 `entriesOf` 转换。
- **时间戳手写 UTC 格式化**，不用 `SimpleDateFormat`：后者依赖默认时区与 Locale，
  会让同一份会话在不同设备上导出不同结果，而导出文件应当稳定。用
  Howard Hinnant 的 civil_from_days 算法（纯整数运算，不依赖时区数据库）。
- **文件名要清理**：标题可能含用户输入的任意字符（路径分隔符、控制字符），直接当文件名
  会失败或写到意外位置；过长也要截断（多数文件系统有长度上限）。
- **UI 导出取当前列表内容而非重读会话日志**：用户看到的就是他要导出的，两者必须一致。

**未做**：PDF/PNG 导出、单条消息的操作（复制/重试/编辑）、分享 Intent（当前只到剪贴板）。
这些需要更多 UI 与平台集成，而剪贴板已覆盖「带走内容」这个主要需求。

**测试**：`ConversationExporterTest` 25 条（标题与默认标题、空/null 列表、null 条目跳过、
空助手内容不产出空标题、工具名列表、状态标记、长输出截断与注明、仅对话模式、
不含输出的模式、压缩记录开关、列表结构、标题前空行、结尾换行、时间戳开关与
确定性格式化（含闰日）、文件名清理与截断、顺序保持）。五个模块共 464 条全绿。

### 3.14 P1-14 自定义 Agent 扩展 + P1-15 Slash 命令（#7）

**新文件**：`core/ai-agent/.../agent/command/` 的 `SlashCommandCatalog`、`CustomAgent`、
`CustomAgentStore`；`core/app` 的 `CustomAgentTool`。
**改动**：助手面板的发送路径先尝试解析斜杠命令；`AgentOrchestrator` 注册自定义 agent
工具；设置页新增「自定义 Agent」项（增删改、启停）。

**关键设计决策**：

- **只有已知命令名才算命令**。用户完全可能输入 {@code /etc/hosts 这个文件是干什么的}。
  未知的 {@code /xxx} 一律按普通消息处理，由模型去理解——弹一个「未知命令」错误会让
  这条消息发不出去，而用户本来就没打算用命令。
- **补全判断空白要用未 trim 的原文**：{@code "/mode "} 的末尾空格是有意义的信号
  （用户已打完命令名），trim 会把它抹掉，于是参数阶段仍在提示命令名、遮住参数输入。
- **参数缺失时列出可选值**，而不是只说「参数错误」。{@code /mode} 无参数时直接给出
  {@code chat|plan|agent|control}，用户立刻就能用起来。
- **`/model` 校验模型名属于当前服务商**：用户手打模型名很可能拼错，直接拒绝并列出可用值，
  比发一个注定失败的请求好。
- **命令不发给模型、不消耗额度**（帮助文本里也这么写，用户才敢用）。
- **自定义 agent 与子 agent 的区别**：子 agent 是模型即席决定派发的上下文隔离；
  自定义 agent 是**用户预先定义**的固定角色（「代码审查员」），模型只提供「要处理什么」。
  用户因此能固化项目特定要求，不必每次在对话里重复说明。
- **自定义 agent 名字要规整成工具安全标识**：名字会拼进 {@code agentx_<名字>}，
  而工具名会进入模型的 tools 定义。空格、标点、斜杠会让它难以被稳定复述；
  但常见的分隔符转成下划线而非丢弃（丢弃会把「code reviewer」变成「codereviewer」）。
  中文名字保留（用户很可能用中文命名）。
- **名字即标识**：存储按名字索引，同名覆盖天然实现「编辑」；同名两条定义会让模型看到
  的行为随机。编辑时名字不可改——改名等于换一个工具。
- **每次运行读取自定义 agent**：用户改完定义后下一次运行即生效，不必重启。
- **名字冲突时跳过注册**：覆盖会让另一个自定义 agent 静默消失。
- **自定义 agent 归类为 WRITE**：其内部工具集由实现方决定，可能写文件。保守归类比
  放行一个可能写文件的工具更安全。
- **设置界面提供启停而非只有删除**：临时关掉一个 agent 不该丢掉它的提示词。

**未做**：自定义 agent 的导入导出、按 agent 覆盖模式/模型、斜杠命令的图形化弹出列表
（当前只在输入框内解析，补全能力已由 `matching` 提供但未接 UI）。

**测试**：`SlashCommandCatalogTest` 27 条 + `CustomAgentStoreTest` 23 条
（命令与普通消息的边界、大小写、空白、模式与模型参数校验、补全前缀、
定义与解析的一致性、名字规整、查重覆盖、上限、启停、跨实例持久化、损坏文件容错）。
五个模块共 511 条全绿。

### 3.15 P1-13 Skill 系统（#6）

**新文件**（`core/ai-tool/.../tool/skill/`）：`Skill`、`SkillParser`、`SkillRegistry`、
`SkillTool`。
**改动**：`AgentOrchestrator` 构造时加载 skill 目录、注册工具、把 skill 清单注入提示词；
设置页新增「Skill」项（展示已加载的 skill 与目录路径）。

**关键设计决策**：

- **渐进披露是核心**。系统提示词的每个字都会进入**每一次**请求；项目里可能有十几条
  「这个模块的约定」，全部写进提示词会永久占用上下文，而其中绝大多数与当前任务无关。
  因此提示词里只放名字与一句话说明（每条约 20 token），模型判断需要时再用 `skill`
  工具读取完整正文。几十个 skill 的常驻成本只有几百 token。
- **格式用 Markdown + frontmatter**，与静态站点生成器一致——用户很可能已经熟悉这种写法。
- **手写解析而非引入 YAML 库**：只需要 `key: value` 两种标量，而 YAML 的完整语法
  （锚点、多行折叠、类型推断）远超需求，引入依赖只为解析两行不值。
- **只认文件开头的 `---` 块**。Markdown 的分隔线就是 `---`，把正文中间的它当边界会
  截断文档——一个含分隔线的普通说明会变成残缺的说明，而模型会照着残缺内容工作。
- **没有 frontmatter 时尽力解释而非报错**：用文件名当名字、正文首行当说明。用户很可能
  直接写一个 Markdown 就丢进来，报错拒绝不如尽力解释。
- **`SKILL.md` 用目录名当名字**：这个文件名本身没有信息量，而 skill 的标识通常是目录名。
  frontmatter 里的 `name` 仍然优先。
- **支持两种布局**（`*.md` 与 `*/SKILL.md`）：两种写法都常见，单文件适合短说明、
  子目录适合带附件的复杂 skill。
- **重名时先加载的胜出**（按路径排序）：顺序稳定，用户能预期哪个生效；
  「后加载覆盖」会让结果依赖文件系统的遍历顺序，不可预期。
- **坏文件跳过**：读不出来、无正文、名字非法都只跳过该文件，一个坏文件不该让全部
  skill 不可用。
- **正文超限截断并注明**：静默截断会让模型以为拿到了全部约定，照着残缺内容写代码。
- **找不到 skill 时列出可用名字**：模型拼错一个字母就白跑一轮，给出候选能省下这一轮。
- **设置页展示实际加载结果与目录路径**：「放进去了但没生效」是最常见的困惑
  （文件名不对、frontmatter 格式错、正文为空），让用户能自行定位。

**未做**：在线 skill 商店（`SkillHubClient` / `GitHubSkillInstaller`）——它是外部内容
分发渠道，需要额外的信任模型（下载并执行他人写的指令有风险）；skill 的调用统计
（`skill_usage` 表）。当前是本地文件 + 按需加载，已覆盖「把项目约定外置」这个主要价值。

**测试**：`SkillTest` 33 条（frontmatter 解析、降级取文件名与首行、正文中间分隔线不误判、
未闭合 frontmatter、引号剥离、注释与未知键、CRLF、前导空行、超长截断、名字校验、
两种目录布局、frontmatter 覆盖目录名、大小写不敏感、重名稳定胜出、坏文件跳过、
非 Markdown 忽略、数量上限、提示词不含正文、工具加载/列出/找不到时的候选、
只读属性）。五个模块共 544 条全绿。

### 3.16 P2 收尾项（#9）——日志脱敏

**P2 清单里其余各项已有现成实现，无需移植**：

| 项 | 现状 |
|---|---|
| 错误日志中心 | `IDELogFragment` / `AppLogFragment` 已存在；`ErrorLog.setSink` 在 `IDEApplication` 里接到 root logger，而 fragment 也挂在 root logger 上，因此 AI 错误**已经**出现在 IDE 日志界面 |
| 代理设置 | `AppProxy` / `UrlPolicy` 已移植，`SimpleHttpClient` 已在用 |
| 主题系统 | ACS 有完整的 Material3 主题；LCP 的 `LineTheme` 是它自有体系，移植收益为负 |
| 归档导入导出 | LCP 专有格式，无既有用户数据可导 |
| 输入设置 / 附件 | 需要平台集成（文件选择器、附件管理），规模超出一个收尾项 |

**实际修掉的缺陷：日志未脱敏**。

`ErrorLogRedactor` 此前在 `ai-protocol` 里，但**只有协议层的一个调用点用了它**：

- 协议层把 `throwable.getMessage()` 当 `summary` 传，未脱敏——而异常消息里可能带含密钥的
  URL，或服务端回显的请求头；
- 工具层（`ToolExecutor`）把**原始工具输出**当 `details` 传，完全未脱敏——而用户文件里
  写着 `api_key = "..."` 是常事；
- app 层的 sink 直接把 `details` 写进日志。

**改动**：把脱敏器移到 `ai-tool-api`（`ai-protocol` 已依赖它，因此两边都能用），
并在 `ErrorLog.record` 这个**唯一入口**上对四个字段全部脱敏，删掉旧的重复实现。

**关键设计决策**：

- **脱敏放在漏斗而非各调用点**。「每个调用方都记得脱敏」在现实中做不到，漏一处就是一次
  凭据泄露——而日志会被贴进 issue、被上报、被截图。
- **`redactThrowable` 派生包装异常而非改原对象**：直接改 `getMessage()` 会影响到还在向上
  传播的同一个异常，可能破坏上层判断。仅当消息确实含敏感内容时才包装——无谓的包装会让
  日志里的异常类型变化，给排查增加噪声。
- **补上查询串里的密钥**：`?key=` / `&token=` 这类形式在 MCP server 地址与部分模型端点里
  很常见，而它们常出现在异常消息中。旧实现没有覆盖。
- **保留非敏感字段**：脱敏不能把日志变成一堆标记，否则失去诊断价值。测试里专门钉住
  「普通内容原样通过」与「短 base64 样串不误伤」。

**测试**：`ErrorLogRedactorTest` 20 条（Authorization 头、api-key 头、JSON 敏感字段八种
命名、查询串密钥、data URL base64、b64_json、普通内容不误伤、短串不误伤、null/空、
超长截断、异常脱敏与原因链保留、无敏感内容时不包装、`ErrorLog` 四字段全部脱敏、
sink 抛异常不影响主流程）。

> ⚠️ 这里的 `AUTHORIZATION` / `API_KEY_HEADER` 两条正则**写错过**（量词用了惰性 `+?`），
> 已由 §3.17 修正。初版测试没发现的原因是断言方式：只检查「密钥整串是否还在」，
> 而惰性匹配会产出 `[REDACTED]k-abcdef...`——整串确实不在了，密钥却只掉了第一个字符。

### 3.17 移植后安全审查与修正

移植完成后对全部 14 个提交（约 17.6k 行）做了一次独立安全审查，**发现 4 处需要修的问题**。
其中两处（H1、H2）直接推翻了 §3.4 的设计，另两处（H4、M1）是移植自 LineCode Pro 的既有缺陷。

**H1 — `file_delete` 授权范围窄于实际删除集合**（提权）
`ToolPermissionRule` 只读 `paths` 数组，而 `FileDeleteTool` 还接受 `file_path` / `path`。
用户对 `paths:["A"]` 点「始终允许」后，模型给出
`{"paths":["A"],"file_path":"B"}` 会命中同一条规则、**不再弹窗**，然后两个文件都被删。
修法不是「把两处都改对」，而是**让两者共用同一个方法**：`FileDeleteTool.collectPaths`
改为 `public static`，`ToolPermissionRule` 直接调用它。授权与执行从此不可能漂移。

**H2 — shell 按「首词」授权等于按解释器授权**（提权）
`git status` 与 `git push --force`、`bash build.sh` 与 `bash -c 'cat ~/.ssh/id_rsa'`、
`python3 x.py` 与 `python3 -c "..."` 全部同键——用户点一次「始终允许」就等于把整个
git 或整个解释器交出去。类注释当时**写的是反话**（声称「不会顺带放行 `git push --force`」），
而测试 `assertEquals(gitStatus, gitPush)` 把这个提权路径钉成了「正确行为」。
修法：scope 取归一化后的**整条命令**（仅压空白，不折大小写——shell 区分大小写）。

**M5 — 空 scope 产生「无限放行」规则**
`scopeOf` 对无判别字段的工具返回空串，拼出 `<tool>\0` 键，对该工具**任意参数**都匹配。
受影响的是 `mcpx_*`、`agent`、`agentx_*`、`install_apk`、`gradle_build`——
恰恰是设计上最想收紧的那批。修法：改取参数摘要 `args:<sha256 前 8 字节>`。

**H4 — HTTP 重定向不逐跳校验**（SSRF）
`setInstanceFollowRedirects(true)` 只在初始 URL 上过一次 `UrlPolicy`，之后每一跳都由
JDK 内部跳转，不再回到校验点。一个正常的公网地址回
`302 Location: http://192.168.1.1/` 就能把请求打到内网，而调用方以为在读公网文档。
修法：关闭自动跟随，改为手动循环，**每一跳都重新校验**；同时按 HTTP 语义处理
方法与请求体（303 与 301/302+POST 降级为 GET 并丢弃请求体，307/308 保持原样），
上限 5 跳。

**M1 — 脱敏正则只吃掉密钥的一个字符**
`+?` 惰性量词在匹配到一个字符后立刻收手，`Authorization: Bearer sk-abcdef` 变成
`Authorization: Bearer [REDACTED]k-abcdef`——密钥只是掉了首字符，其余原样进日志。
修法：改贪婪量词，靠 `[^\r\n,}]` 收尾。测试改为**同时断言尾段与尾部片段不残留**。

**测试**：`ToolPermissionRuleTest` 新增/改写 8 条（整条命令粒度、解释器参数隔离、
夹带 `file_path` 的删除必须不同键、空 scope 不得出现），
新增 `SimpleHttpClientRedirectTest` 5 条（拒绝跳向不允许的主机、第二跳也校验、
合法链可跟随、跳数上限、POST 重定向丢弃请求体）。

> **这一节最值得记的不是四个修复，而是两个测试教训**：
> 1. **期望值必须来自威胁模型，不能来自当前实现**。H2 的错误被测试钉成「正确」，
>    因为它把 `assertEquals(gitStatus, gitPush)` 当成了不变量。
> 2. **断言要落在「为什么被拒」上，不能只断言「抛异常」**。H4 的测试若只写
>    `assertThrows(Exception.class)`，修复前后都通过——旧实现真的去连内网域名，
>    然后因 DNS 失败抛 `UnknownHostException`。改为断言 `IllegalArgumentException`
>    且消息含 `cleartext`（即「请求发出之前就被策略拦下」），测试才有判别力。
>    已验证：临时把 `setInstanceFollowRedirects` 改回 `true`，这两条测试立刻失败。

五个模块共 571 条全绿（1 条跳过：`AgentLiveGatewayTest` 需要 `-Dai.live.baseUrl`）。
...
`agent_output`、在线 skill 商店、向量检索、PDF/PNG 导出等），它们不是遗漏，
而是判断为「收益低于改动规模」或「依赖外部分发渠道需额外信任模型」。
...
| #6 | P1-13 Skill 系统 | **已完成** | — | 见 §3.15；在线商店未做 |
| #13 | P1-10 消息操作与导出 | **已完成** | — | 见 §3.13 |
| #7 | P1-14 自定义 Agent 扩展 + P1-15 Slash 命令 | **已完成** | — | 见 §3.14 |
| #9 | P2 收尾项（日志/归档/代理/输入/主题） | **已完成** | — | 见 §3.16 |
